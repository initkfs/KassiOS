 ...
 * Authors: initkfs
 */
module kernel_legacy;

version (legacy)
{

    mixin template KernelLegacy()
    {
        import os.std.tests;
        ...
        extern (C) void kmain(size_t magic, size_t* multibootInfoAddress)
        {
            ...
            if (Syslog.isTraceLevel)
            {
                ...
                immutable memArgs = [
                    cast(size_t) memoryStart, cast(size_t) memoryEnd
                ].staticArr;
            ...
            foreach (Multiboot.multiboot_tag* tag; Multiboot.createTagIterator(
                    multibootInfoAddress))
            {
                switch (tag.type)
                {
                case MultibootSpec.MULTIBOOT_TAG_TYPE_CMDLINE:
                    ...
                    if (Syslog.isTraceLevel)
                    {
                        if (cmdLine.length > 0)
                        {
                            ...
                            const bool isAcpi = !Strings.contains(cmdLine, CoreConfig
                                    .noAcpiKernelArgKey);
                            if (Syslog.isTraceLevel)
                            {
                                if (isAcpi)
        ...
        extern (C) __gshared void runInterruptServiceRoutine(const ulong num, const ulong err)
        {
            ...
            case Isr.Exception.PageFault:
                ...
                auto errMessagePtr = Strings.format("Page fault: %x", [
                        errorAddr
                    ].staticArr);
                scope (exit)
                {
